...
import { toolsEn, categoryOrderEn, getToolsByCategoryEn, type ToolEn } from "@/lib/tools-en";
import { matchIntentEn, getQuickSuggestionsEn } from "@/lib/ai-router-en";
...
function ToolCard({ tool }: { tool: ToolEn }) {
  return (
    <Link
...
function HomeContent() {
  ...
  const [aiMatch, setAiMatch] = useState<ReturnType<typeof matchIntentEn>>(null);
  const [isTyping, setIsTyping] = useState(false);

  const filteredTools = useMemo(() => {
    if (!query.trim()) return getToolsByCategoryEn();
    const q = query.toLowerCase();
    const filtered = toolsEn.filter(
      (t) =>
  ...
  const suggestions = getQuickSuggestionsEn();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const match = matchIntentEn(query);
      setAiMatch(match);
      setIsTyping(false);
    }, 300);
  ...
  const navigateToTool = useCallback((slug: string) => {
    window.location.href = `/en/tools/${slug}`;
  }, []);
      ...
      <section className="relative flex min-h-[90vh] flex-col items-center justify-center px-4">
        ...
        <div className="relative z-10 mx-auto w-full max-w-3xl text-center">
          ...
          >
            ...
            20+ Free Tools · Local Processing · Private & Secure
          </div>
            ...
            <div className="relative">
              ...
              <div className="relative flex items-center rounded-2xl border border-zinc-800 bg-zinc-900/90 backdrop-blur-xl">
                ...
                <input
                  ...
                  placeholder="What do you need? e.g. format JSON, generate password, test regex..."
          ...
          >
            <StatBadge value="20+" label="Free Tools" />
            <div className="w-px bg-zinc-800" />
            <StatBadge value="0ms" label="Instant Load" />
            <div className="w-px bg-zinc-800" />
            <StatBadge value="100%" label="Client-Side" />
          </div>
        </div>
        ...
        >
          <div className="flex flex-col items-center gap-2 text-zinc-600">
            <span className="text-xs">Browse All Tools</span>
        ...
        <div className="mx-auto max-w-6xl">
          {categories.length === 0 ? (
            <div className="py-32 text-center">
              ...
              <p className="text-lg text-zinc-500">No matching tools found</p>
              <p className="mt-2 text-sm text-zinc-600">Try a different search? e.g. "format JSON" or "generate password"</p>
            </div>
          ) : (
