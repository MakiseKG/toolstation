...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: {
      canonical: `https://toolstation-sooty.vercel.app/en/cheatsheets/${slug}`,
      languages: {
...
export default async function CheatSheetPageEN({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
    ...
    <div className="min-h-screen bg-[#030305]">
      ...
      <div className="relative mx-auto max-w-4xl px-4 py-8">
        ...
        <nav className="mb-8 flex items-center gap-2 text-sm text-zinc-600">
          <Link href="/en" className="flex items-center gap-1.5 transition hover:text-[#00d4aa]">
          ...
          <Link href="/en/cheatsheets" className="transition hover:text-[#00d4aa]">
            Cheat Sheets
          </Link>
          <svg className="h-3.5 w-3.5 text-zinc-700" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        ...
        {/* Content */}
        <CheatSheetContent sheet={sheet} locale="en" />

        {/* Related cheatsheets */}
        <section className="mt-16">
          <h2 className="mb-6 text-lg font-bold text-zinc-300">More Cheat Sheets</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            {relatedSheets.map((s) => (
              <Link
                key={s.slug}
                href={`/en/cheatsheets/${s.slug}`}
